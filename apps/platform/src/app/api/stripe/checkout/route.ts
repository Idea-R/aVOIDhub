...
import { hasApprovedCreatorApplication } from "@/lib/creators/server";
...
export async function POST(request: NextRequest) {
  ...
  if (!user)
    return NextResponse.json(
      { error: "authentication_required", login: "/login/" },
      { status: 401 },
    );
  ...
  if (
    parsed.data.plan === "creator" &&
    !(await hasApprovedCreatorApplication(user.id))
  ) {
    return NextResponse.json(
      { error: "creator_approval_required", apply: "/creators/apply/" },
      { status: 403 },
    );
  }
  const priceId = getStripePriceId(parsed.data.plan);
  if (!priceId)
    return NextResponse.json({ error: "membership_not_open" }, { status: 503 });
