...
export function GameCard({ game, index, compact = false }: GameCardProps) {
    ...
    <>
      ...
      >
        ...
        {game.hosting === "independent" ? (
          <span className="gameCaptureLabel">Live site capture</span>
        ) : null}
      ...
      <div className="gameCardCopy">
        ...
        <span className="gameAction" aria-hidden="true">
          {game.status === "soon" ? "Hold position" : "View game"}
          {game.status === "soon" ? <Clock3 /> : <ArrowRight />}
        </span>
      </div>
    </>
  ...
  if (game.status === "soon") {
    return (
      <article
        className={`gameCard gameCardSoon ${compact ? "gameCardCompact" : ""}`}
        aria-disabled="true"
      >
        <span className="soonSlash" aria-hidden="true">
          COMING SOON
        </span>
        {content}
      </article>
  ...
  return (
    <Link
      ...
      aria-label={`${game.title}: view game details`}
