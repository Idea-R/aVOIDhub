...
export function CreatorApplicationForm({ enabled }: { enabled: boolean }) {
  ...
  return (
    <form className="platformForm" onSubmit={submit}>
      ...
      <input id="displayName" name="displayName" minLength={2} maxLength={60} required />
      <label htmlFor="portfolioUrl">Portfolio or game URL <span>optional</span></label>
      <input id="portfolioUrl" name="portfolioUrl" type="url" placeholder="https://" />
      <label htmlFor="pitch">What do you make, and what belongs here?</label>
      <textarea id="pitch" name="pitch" minLength={40} maxLength={2000} rows={7} required />
      <button className="primaryButton" type="submit" disabled={!enabled}>Submit for review</button>
      <p className="formStatus" aria-live="polite">{status || (enabled ? 'Submissions are reviewed before profiles, hosting, or ads are enabled.' : 'Applications open with account access.')}</p>
    </form>
  )
}
