      ...
      if (dist < ballRadius + enemy.size) {
        ...
        if (activeEffects.electrified) {
          ...
          const lightningTargets = enemies.filter(
            (nearbyEnemy, nearbyIndex) =>
              nearbyIndex !== enemyIndex &&
              PhysicsEngine.distance(enemy.pos, nearbyEnemy.pos) < 120,
          );
  ...
  static checkChainEnemyCollisions(
    ...
    chain.forEach((segment) => {
      enemies.forEach((enemy, enemyIndex) => {
        ...
        if (
          enemy.lastChainHitTime &&
          currentTime - enemy.lastChainHitTime < chainImmunityDuration
        ) {
  ...
  static checkPlayerEnemyCollisions(
    ...
    enemies.forEach((enemy, enemyIndex) => {
      ...
      if (dist < playerSize + enemy.size) {
        ...
        if (activeEffects.berserk) {
          damage = Math.ceil(
            damage * activeEffects.berserk.vulnerabilityMultiplier,
          );
        }
