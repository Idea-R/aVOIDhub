...
export class EnemyManager {
  ...
  spawnEnemy(
    ...
    const types: Exclude<Enemy["type"], "boss">[] = ["basic"];
    ...
    if (wave >= 8 && Math.random() < 0.05) {
      // 5% chance for ninja star after wave 8
      types.push("ninja_star");
    }
    if (wave >= 12 && Math.random() < 0.08) {
      // 8% chance for pusher after wave 12
      types.push("pusher");
    }
  ...
  spawnBoss(
    ...
    const ufoTypes: NonNullable<Enemy["ufoType"]>[] = [
      "scout",
      "destroyer",
      "mothership",
      "harvester",
  ...
  updateBossShooting(playerPos: Vector2): void {
    ...
    this.enemies.forEach((enemy) => {
      if (
        enemy.type === "boss" &&
        enemy.lastShotTime !== undefined &&
        enemy.shootCooldown !== undefined
      ) {
  ...
  private createSingleProjectile(
    boss: Enemy,
    playerPos: Vector2,
    speed: number,
  ): void {
  ...
  private createRapidProjectiles(
    boss: Enemy,
    playerPos: Vector2,
    speed: number,
    count: number,
  ...
  updateBossMinions(playerPos: Vector2, maxOrdinaryEnemies: number): void {
    ...
    this.enemies.forEach((enemy) => {
      if (
        ...
        if (
          currentTime - enemy.lastMinionSpawnTime >
          enemy.minionSpawnCooldown
        ) {
          // Spawn minions around the boss
          const minionCount = enemy.bossAbilities?.includes("elite_minions")
            ? 3
            : 2;
  ...
  update(
    ...
    this.enemies = this.enemies.filter((enemy) => {
      return (
        enemy.health > 0 && // Ensure dead enemies are removed
        enemy.pos.x > -150 &&
        enemy.pos.x < canvasWidth + 150 &&
        enemy.pos.y > -150 &&
        enemy.pos.y < canvasHeight + 150
      );
    });
    ...
    this.projectiles = this.projectiles.filter((projectile) => {
      return (
        projectile.life > 0 &&
        projectile.pos.x > -50 &&
        projectile.pos.x < canvasWidth + 50 &&
        projectile.pos.y > -50 &&
        projectile.pos.y < canvasHeight + 50
      );
    });
  }
    ...
    switch (pattern) {
      case "evasive": {
        ...
        enemy.velocity.x =
          ...
          perpDirection.x * baseSpeed * 0.8 * dartDirection;
        enemy.velocity.y =
          direction.y * baseSpeed +
          perpDirection.y * baseSpeed * 0.8 * dartDirection;
        break;
      }
