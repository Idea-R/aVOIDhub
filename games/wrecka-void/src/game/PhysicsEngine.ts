...
export class PhysicsEngine {
  ...
  static updateChainPhysics(
    ...
    const damping = 0.998;
    const targetDistance = isRetracting
      ? chainSegmentDistance * 0.6
      : chainSegmentDistance;
  ...
  static updateBallPhysics(
    ...
    const ballTargetDistance = isRetracting
      ? chainSegmentDistance * 0.6
      : chainSegmentDistance;
  ...
  static updateEnemies(
    ...
    enemies.forEach((enemy) => {
      ...
      if (
        enemy.hasEnteredPlayArea ||
        (enemy.pos.x > 0 &&
          enemy.pos.x < canvasWidth &&
          enemy.pos.y > 0 &&
          enemy.pos.y < canvasHeight)
      ) {
        ...
        if (enemy.pos.x <= padding || enemy.pos.x >= canvasWidth - padding) {
          ...
          enemy.pos.x = Math.max(
            padding,
            Math.min(canvasWidth - padding, enemy.pos.x),
          );
        }
        if (enemy.pos.y <= padding || enemy.pos.y >= canvasHeight - padding) {
          ...
          enemy.pos.y = Math.max(
            padding,
            Math.min(canvasHeight - padding, enemy.pos.y),
          );
        }
      }
  ...
  static updateSecondChainPhysics(
    ...
    const rotationSpeed =
      baseRotationSpeed * (1 + playerUpgrades.secondChainSpeed * 0.5);
