...
export class EnemyRenderer {
  ...
  private drawEnemyHealthBar(enemy: Enemy): void {
    if (enemy.health < enemy.maxHealth) {
      ...
      this.ctx.fillRect(
        enemy.pos.x - barWidth / 2,
        enemy.pos.y - enemy.size - 15,
        barWidth,
        barHeight,
      );
  ...
  private drawUFOBoss(
    ...
    this.drawUFOEffects(pos, size, bossAbilities);
  ...
  private drawPusher(enemy: Enemy): void {
    ...
    spikePositions.forEach((spike) => {
      ...
      this.ctx.lineTo(
        spike.x + (spike.x - pos.x) * 0.5,
        spike.y + (spike.y - pos.y) * 0.5,
      );
      this.ctx.stroke();
    });
  }
