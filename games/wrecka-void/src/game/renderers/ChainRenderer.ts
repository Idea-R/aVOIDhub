...
import { WRECK_COSMETICS, type WreckCosmeticId } from "../../cosmetics";
...
export class ChainRenderer {
  ...
  drawChain(
    ...
    const palette = WRECK_COSMETICS[cosmetic];
    let chainColor = isRetracting ? palette.link : palette.chain;
    ...
    chain.forEach((segment, index) => {
      ...
      this.ctx.fillStyle = isRetracting ? palette.player : palette.link;
      this.ctx.strokeStyle = isRetracting ? palette.chain : palette.ballEdge;
  ...
  drawSecondChain(
    ...
    if (secondChain.segments.length > 0) {
      this.ctx.moveTo(
        secondChain.segments[0].pos.x,
        secondChain.segments[0].pos.y,
      );
      for (let i = 1; i < secondChain.segments.length; i++) {
        this.ctx.lineTo(
          secondChain.segments[i].pos.x,
          secondChain.segments[i].pos.y,
        );
      }
  ...
  private drawChainSpikes(
    position: Vector2,
    linkRadius: number,
    chainDamageLevel: number,
    isSecondChain = false,
  ): void {
