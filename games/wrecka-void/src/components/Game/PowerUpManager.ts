import { PowerUp, Vector2, PlayerUpgrades } from "../../types/PowerUps";

type PowerUpDefinition = Pick<
  PowerUp,
  "category" | "name" | "color" | "effect" | "duration"
>;
type GeneratedPowerUp = PowerUpDefinition & Pick<PowerUp, "type">;
...
export class PowerUpManager {
  ...
  spawnPowerUp(
    ...
    if (rarityRoll < 0.08 + wave * 0.008) {
      // 8% + 0.8% per wave, max ~16%
      rarity = "very_rare";
    } else if (rarityRoll < 0.25 + wave * 0.015) {
      // 25% + 1.5% per wave, max ~40%
      rarity = "rare";
    } else {
      rarity = "common";
    }
  ...
  private generatePermanentPowerUp(
    rarity: PowerUp["rarity"],
    playerUpgrades: PlayerUpgrades,
  ): GeneratedPowerUp {
    const permanentTypes: Record<PowerUp["rarity"], PowerUpDefinition[]> = {
      common: [
        {
          category: "health",
          name: "Healing Heart",
    ...
    if (rarity === "very_rare") {
      const megaSpikesAvailable =
        playerUpgrades.ballDamage >= 3 && playerUpgrades.chainDamage >= 3;
      const secondChainAvailable = playerUpgrades.chainExtensions >= 2; // Need max extensions first
      const chainMasteryAvailable =
        playerUpgrades.hasSecondChain && playerUpgrades.secondChainDamage < 3;
      const chainVelocityAvailable =
        playerUpgrades.hasSecondChain && playerUpgrades.secondChainSpeed < 2;
    ...
    if (rarity === "rare") {
      options = options.filter((option) => {
        if (option.name === "Chain Extension")
          return playerUpgrades.chainExtensions < 2;
        return true;
      });
    }
  ...
  private generateTemporaryPowerUp(
    rarity: PowerUp["rarity"],
  ): GeneratedPowerUp {
    const temporaryTypes: Record<PowerUp["rarity"], PowerUpDefinition[]> = {
      common: [
        {
          category: "speed",
          name: "Speed Burst",
  ...
  update(
    ...
    this.powerUps = this.powerUps.filter((powerUp) => {
      return (
        powerUp.pos.x > -100 &&
        powerUp.pos.x < canvasWidth + 100 &&
        powerUp.pos.y > -100 &&
        powerUp.pos.y < canvasHeight + 100
      );
    });
  }
  ...
  spawnBossPowerUp(pos: Vector2, playerUpgrades: PlayerUpgrades): void {
    ...
    this.spawnSpecificPowerUp(pos, playerUpgrades, "permanent", rarity);
  }
}
