...
export class GameLogic {
  ...
  constructor(canvas: HTMLCanvasElement, systems: GameSystems, settings: GameSettings, spatialGrid: SpatialGrid) {
    this.canvas = canvas;
    this.systems = systems;
    this.settings = settings;
    
    // Receive spatial grid from EngineCore (single source of truth)
  ...
  updateSpatialGrid(width: number, height: number): void {
    // Spatial grid is now managed by EngineCore, just update dependent systems
    this.systems.collisionSystem.updateSpatialGrid(this.spatialGrid);
  ...
  /**
   * Get the spatial grid instance (backwards compatibility)
   * @returns SpatialGrid instance
   */
