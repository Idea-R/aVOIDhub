...
import { User, Trophy, Target, Clock, Zap, Calendar, ExternalLink, Eye, EyeOff, Twitter, Instagram, Youtube, Twitch, Github } from 'lucide-react';
...
interface ProfileInfoProps {
  ...
  onProfileUpdate: (field: keyof UserProfile, value: any) => void;
}

export function ProfileInfo({ profile, isOwnProfile, onProfileUpdate }: ProfileInfoProps) {
  return (
...
export function SocialLinksEditor({ socialLinks, socialErrors, onSocialLinkChange }: SocialLinksEditorProps) {
  const getSocialIcon = (platform: string) => {
    const icons = {
      twitter: Twitter,
      instagram: Instagram,
...
export function PrivacySettings({ isPublic, onTogglePrivacy }: PrivacySettingsProps) {
  return (
    <div className="space-y-6">
      <div className="bg-gray-800 rounded-lg p-4">
