...
export class UnifiedAuthService {
    ...
    currentSession = null;
    authListeners = [];
    STORAGE_KEY = 'avoidgame_auth_session';
    constructor() {
    ...
    getCurrentGame() {
        ...
        if (hostname.includes('voidavoid') || pathname.includes('/void'))
            return 'voidavoid';
        if (hostname.includes('wreckavoid') || pathname.includes('/wreck'))
            return 'wreckavoid';
        if (hostname.includes('tankavoid') || pathname.includes('/tank'))
            return 'tankavoid';
        return 'hub';
    }
    async getOrCreateUnifiedUser(user) {
        try {
    ...
    async trackGameSession(userId, game) {
        try {
            await this.supabase
                .from('game_sessions')
                .insert([{
    ...
    async saveLeaderboardScore(game, score) {
        if (!this.currentSession?.user)
            return { success: false, error: 'Not authenticated' };
        try {
            const { error } = await this.supabase
