export class UnifiedLeaderboardSystem {
    static instance;
    supabase; // Will be injected
    leaderboardConfigs = new Map();
    listeners = new Map();
    cache = new Map();
    constructor() {
    ...
    async submitScore(userId, gameKey, score, metadata) {
        if (!this.supabase) {
    ...
    async getGameLeaderboard(gameKey, limit = 50, offset = 0) {
        if (!this.supabase) {
            return { success: false, error: 'Supabase client not initialized' };
        }
    ...
    async getGlobalLeaderboard(limit = 50, offset = 0) {
        ...
        try {
            ...
            const globalScores = data.map((row, index) => ({
                userId: row.user_id,
                username: row.username,
                userAvatar: row.user_avatar,
    ...
    async getUserRank(userId, gameKey) {
        if (!this.supabase) {
            return { success: false, error: 'Supabase client not initialized' };
        }
        try {
    ...
    async calculateRank(gameKey, score) {
        const config = this.leaderboardConfigs.get(gameKey);
        if (!config)
            return -1;
        const ascending = config.scoreType === 'low';
    ...
    async updateGlobalLeaderboard(userId) {
        if (!this.supabase)
            return;
        try {
            // Get all user scores
            ...
            const gameGroups = userScores.reduce((acc, score) => {
                if (!acc[score.game_key] || score.score > acc[score.game_key]) {
                    acc[score.game_key] = score.score;
                }
                return acc;
            }, {});
            Object.entries(gameGroups).forEach(([gameKey, score]) => {
