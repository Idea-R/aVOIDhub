...
Review the stacked hosted-game branches and TankaVOID T0–T5. The next TankaVOID slice is T6: connect its existing platform detail surface to optional platform identity, one-use runs, honest result receipts, personal best, and a provisional board without direct browser score writes. Physical touch certification, final balance/performance, deployed smoke, and rollback remain T7. VOIDaVOID V3, WreckaVOID W3, WORDaVOID WD2, executable WD1 run-ticket acceptance, and data-backed platform work remain gated on the short-lived Supabase development branch. Production data, secrets, Stripe, AdSense, DNS, and production deploys remain untouched.
