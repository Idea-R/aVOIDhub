...
36. **Stack consolidation and launch program — in progress.** PR #45 is merged. The 17-request game stack is being reconciled onto current `main` in one integration branch so the current platform design, membership, auth, WreckaVOID repairs, game detail surfaces, WORDaVOID/VOIDaVOID hardening, TankaVOID release candidate, contracts, tests, and leaderboard work can ship from one coherent commit graph.

## Current next action

Complete and verify the launch integration against current `main`, publish one Git-driven Netlify preview, and close the superseded stacked requests after the integrated branch lands. Then exercise the coordinated Supabase migration and run-ticket matrix in an isolated environment, finish the Wreck Run balance/director work, expose the TankaVOID Proving Grounds Play route after device/deploy/rollback evidence, and test Stripe membership plus cosmetic entitlements end to end. Production secrets, charges, AdSense requests, and schema changes remain gated until their exact canary evidence passes.
