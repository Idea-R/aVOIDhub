...
Publish the shared-identity candidate to a Git-driven preview, then exercise the coordinated Supabase migration and run-ticket matrix in an approved isolated environment. Use that canary to certify passwordless login continuity, WreckaVOID and TankaVOID results, profiles, and noncompetitive cosmetic entitlements. Stripe test-mode membership follows the data canary. Expose TankaVOID Proving Grounds Play only after physical-device, deployed-route, and rollback evidence. Production secrets, charges, AdSense requests, and schema changes remain gated until their exact canary evidence passes.
