...
- Active integration worktree: `C:\dev\_worktrees\aVOID\launch\games-v1`
- Active integration branch: `codex/feature-launch-wreck-tank`
- Launch pair: WreckaVOID and TankaVOID
- WreckaVOID V1 plan: `games/wrecka-void/docs/V1-DELIVERY-PLAN.md`
...
- Active TankaVOID T7 branch: `codex/tankavoid-t7-release-hardening`
- Active TankaVOID T7 issue: `https://github.com/Idea-R/aVOIDhub/issues/36`
- Active TankaVOID T7 draft PR: `https://github.com/Idea-R/aVOIDhub/pull/37`
- TankaVOID T7 evidence: `docs/sprint-tankavoid-t7.md`
...
32. **TankaVOID T7 release hardening — local candidate gates complete; deployed and physical-device evidence pending.** One root release command and Linux pull-request workflow now verify the shared contract, game, platform, catalog, foundation, bundle, full assembly, review artifact, and absence of a public route. Cumulative frame diagnostics, procedural arena depth, corrected release copy, portrait/short-landscape/desktop browser checks, pause/focus, reduced motion, muted audio, and a 21-start/20-reset soak pass with 4,566 frames at 16.7 ms average, 17 ms maximum, zero long frames, and zero dropped simulation time. The local Netlify adapter failed after a successful application build on a Windows-only dependency-resolution path, so no preview was uploaded. Git-driven preview smoke, rollback evidence, physical iOS/Android, and the T6 database matrix remain release gates. The five-wave build is retained as Proving Grounds if a larger Expedition mode is approved.
33. **Player and Creator membership contract — merged to production source.** Core profile, favorite, play, and eligible leaderboard access remain free. Founding Player adds eligible-page ad suppression, identity, supporter cosmetics as games implement them, and selected experiments. Creator application stays free; paid private submission capacity requires an approved application and active Creator entitlement. Payment cannot approve or publish.
34. **Role-aware platform workspaces — merged to production source.** The passwordless signal gate, player deck, creator workflow, gated private build intake, and administrator control room use server-controlled authorization. Review actions remain operationally narrow and cannot publish, deploy, charge, grant, delete, or activate ads.
35. **WreckaVOID V1 balance charter — accepted for execution.** The measured plan defines a ten-minute three-act Wreck Run, separate Endless Yard, deterministic balance laboratory, encounter budgets, impact-driven damage, meaningful upgrades, measurable completion bands, and W0 through W7 acceptance.
36. **Stack consolidation and launch program — complete.** PR #45 and the complete 17-request game stack landed through PR #46 at `c492933`. The obsolete stacked requests are closed, the exact commit is live on Netlify production, and the current platform design, membership, auth, game details, WreckaVOID repairs, WORDaVOID/VOIDaVOID hardening, TankaVOID candidate, contracts, tests, and leaderboards now share one coherent `main` history.
37. **Wreck Run finite encounter director — local candidate complete.** `wreck-run-v1.0.0-rc.1` replaces score-driven escalation and endless minute bosses with a ten-minute, twenty-wave, three-act run; boss checkpoints, ordinary-enemy caps, projectile caps, boss-phase relief, final-boss overtime, exact boss counts, and a real victory state are now explicit. Cleared-wave and broken-boss bonuses make progress dominate farming. New-score reads are ruleset-scoped and the platform rejects incoherent Wreck result bounds while retaining provisional trust. WreckaVOID passes 37 game tests, 28 platform tests, type/lint/build/budget gates, the complete 29-route platform assembly, and desktop/phone victory-state browser checks.
38. **Shared game identity and first member cosmetics — local candidate complete.** WreckaVOID now uses the platform cookie session and no longer bundles its obsolete local Supabase auth/profile/leaderboard surface. WreckaVOID `Founder ember` and TankaVOID `Founder meteor` are server-entitlement-gated, locally selected, render-only looks. The player-context route, 30 platform tests, 37 Wreck tests, 44 Tank tests, 52 foundation assertions, complete 30-route build, and desktop/portrait/short-landscape browser checks pass. Real unlock, score, and cancellation behavior still require the isolated Supabase and Stripe test canary.
39. **Shared leaderboard source connection — local gate complete; database rehearsal pending.** VOIDaVOID and WORDaVOID now use the platform cookie session and one-use run path. A shared VOID contract lets the server replay survival, meteor, chain, combo, and perfect-knockback scoring from a server seed. Platform boards are separated by mode, show one best row per player, and distinguish server replay from bounded checks without claiming verified anti-cheat. All contract, game, and platform source gates pass; evidence is in `docs/sprint-shared-leaderboards-v1.md`.

## Current next action

Publish the shared-leaderboard candidate to a Git-driven preview. Then exercise the coordinated Supabase migration and run-ticket matrix in an approved isolated environment for VOIDaVOID, WreckaVOID, WORDaVOID, and TankaVOID. Use that canary to certify passwordless login continuity, receipts, per-mode boards, profiles, and noncompetitive cosmetic entitlements. Stripe test-mode membership follows the data canary. Production secrets, charges, AdSense requests, and schema changes remain gated until their exact canary evidence passes.
