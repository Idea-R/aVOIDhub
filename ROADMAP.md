...
Publish the finite Wreck Run candidate for real-play balance feedback, then exercise the coordinated Supabase migration and run-ticket matrix in an approved isolated environment. Use that shared data path to certify WreckaVOID and TankaVOID results, profiles, and noncompetitive cosmetic entitlements; expose the TankaVOID Proving Grounds Play route only after physical-device, deployed-route, and rollback evidence. Stripe test-mode membership follows the data canary. Production secrets, charges, AdSense requests, and schema changes remain gated until their exact canary evidence passes.
