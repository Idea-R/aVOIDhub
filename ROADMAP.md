...
Publish the T7 draft branch and let Netlify build its normal Linux deploy preview, then smoke the held review routes and record the production rollback target. Keep Tanka Coming Soon until physical iOS/Android and the approved short-lived Supabase run-ticket/data matrix pass. In parallel product planning, define the five-wave build as Proving Grounds and scope a larger sortie-based Expedition mode without folding it into the release candidate. VOIDaVOID V3, WreckaVOID W3, WORDaVOID WD2, executable WD1/T6 acceptance, membership activation, and data-backed platform work remain gated on the coordinated branch/runtime. Production data, secrets, Stripe, AdSense, DNS, and production deploys remain untouched.
